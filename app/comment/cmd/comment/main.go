...
func init() {
	flag.StringVar(&flagconf, "conf", server.CommentConfPath, "config path, eg: -conf config.yaml")
}

func newApp(logger log.Logger, gs *grpc.Server, hs *http.Server, r registry.Registrar) *kratos.App {
	return kratos.New(
		kratos.ID(id+"-comment"),
		kratos.Name(Name),
...
func main() {
	...
	go m_logs.InitLog(&logger, id, Name, Version, "comment", false)
	c := config.New(
		config.WithSource(
